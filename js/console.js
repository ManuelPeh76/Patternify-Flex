(function(root, factory) {
    ...
    ([
        ...
        function (module, exports, require) {
            ...
            var comment_handler_1 = require(1);
            var jsx_parser_1 = require(3);
            var parser_1 = require(8);
            var tokenizer_1 = require(15);
            function parse(code, options, delegate) {
                ...
                var parser;
                options && typeof options.jsx === 'boolean' && options.jsx ?
                    parser = new jsx_parser_1.JSXParser(code, options, parserDelegate) :
                    parser = new parser_1.Parser(code, options, parserDelegate);
                var program = isModule ? parser.parseModule() : parser.parseScript();
                var ast = program;
                collectComment && commentHandler && (ast.comments = commentHandler.comments);
            ...
            function tokenize(code, options, delegate) {
                ...
                tokenizer.errorHandler.tolerant && (tokens.errors = tokenizer.errors());
                return tokens;
            }

            exports.tokenize = tokenize;
            var syntax_1 = require(2);
            exports.Syntax = syntax_1.Syntax;
            //exports.console = require(19);
        ...
        function (module, exports, require) {
            ...
            Object.defineProperty(exports, "__esModule", {value: true});
            var character_1 = require(4);
            var JSXNode = require(5);
            var jsx_syntax_1 = require(6);
            var Node = require(7);
            var parser_1 = require(8);
            var token_1 = require(13);
            var xhtml_entities_1 = require(14);
        ...
        function (module, exports, require) {
            "use strict";
            Object.defineProperty(exports, "__esModule", {value: true});
            var assert_1 = require(9);
            var error_handler_1 = require(10);
            var messages_1 = require(11);
            var Node = require(7);
            var scanner_1 = require(12);
            var syntax_1 = require(2);
            var token_1 = require(13);
	...
	function loadPolyfill() {
              !('isConnected' in Node.prototype) && Object.defineProperty(Node.prototype, 'isConnected', {
                  get() {return (!this.ownerDocument || !(this.ownerDocument.compareDocumentPosition(this) & this.DOCUMENT_POSITION_DISCONNECTED));}
              });
...
function tag(tagName, options = {}) {
  ...
  if (!isElement && typeof tagName !== 'string') throw new Error(`${tagName} is invalid value of tag`);
  let el = isElement ? tagName : document.createElement(tagName);
  for (let prop in options) {
    if (options[prop] === undefined) continue;
    if (prop === 'child' && el instanceof Node) el.append(options[prop]);
    else if (prop == 'children' && Array.isArray(options[prop])) el.append(...options[prop]);
    else if (prop == 'attr') for (let p in options[prop]) el.setAttribute(p, options[prop][p]);
    else if (options[prop].constructor.name === 'Object') for (let p in options[prop]) el[prop][p] = options[prop][p];
    else el[prop] = options[prop];
  }
  return el;
}
tag.get=selector=>document.querySelector(selector)?tag(document.querySelector(selector)):null;
tag.getAll=selector=>[...document.querySelectorAll(selector)].map(el=>tag(el));
tag.parse=html=>{const div=tag('div');div.innerHTML=html;return div.childElementCount===1?tag(div.firstElementChild):[...div.children];}
tag.template=function(html,values){if(values){for (let key in values){if (!/^[a-z_][a-z0-9_\-]*$/.test(key)) continue;html=html.replace(new RegExp(`{{${key}}}`, 'g'),values[key]);}}html=html.replace(/{{[a-z_][a-z0-9_\-]*}}/g,'');return html};
exports.tag = tag;
        ...
        /* 19 Console */
        function (module, exports, require) {
        	const esprima = require(0);
        	require(16).loadPolyFill.apply(window);
        	require(17).polyfill.apply(window);
        	const tag = require(18).tag;
            ...
            const $console = tag('c-console', {
                ...
                onclick(e){
                  e.target.getAttribute('action') === "use code" && (
              ...
              function assignCustomConsole() {
                window.console = {
                  assert(condition, msg, ...substituion) {
                    !condition && log('error', getStack(new Error()), msg, ...substituion);
                  },
                  clear() {
                  ...
                  count(hash = 'default') {
                    counter[hash] ? ++counter[hash] : counter[hash] = 1;
                  ...
                  timeEnd(label = 'default') {
                    if (typeof label !== 'string') throw new TypeError('label must be a string');
                    if (!timers[label]) throw new Error(`No such label: ${label}`);
            ...
            function touchend() {
              ...
              document.removeEventListener('mousemove', touchmove, {passive: false});
            ...
            toggler.ontouchstart = touchstart;
            toggler.onmousedown = touchstart;
            toggler.innerHTML = '&#9888;';
            ...
            function getBody(obj, ...keys) {
              ...
              function parseFuntion(data) {
                ...
                if (parsed.type === 'ExpressionStatement') {
                  ...
                  str = expression.type === 'ArrowFunctionExpression' ? joinParams(expression.params, 'arrow') :
                  expression.type === 'FunctionExpression' ? joinParams(expression.params) : parsed.id.name + joinParams(parsed.params || []);
                }
